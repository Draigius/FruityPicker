...
public class Jonction : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        if (iType == 1 || iType == 2)
        {
            ...
            if (iType == 2)
            {
                ...
                funcChangeFlechSens(iType, gameObject, iEtat, 1);
    ...
    private void funcDetectAcroche(GameObject hLanceurFunc)
    {
        ...
        for(int i = 0; i < hLanceurFunc.GetComponent<Jonction>().iIdEnfant.Length; i++)
        {
            ...
            for (int j = 1; j < hMainCam.GetComponent<MainGame>().hTableJunction.Length; j++)
            {
                Debug.Log("j :" + j);

                int iIdEnVerification = hMainCam.GetComponent<MainGame>().hTableJunction[j].GetComponent<Jonction>().iIdActuel;

                if (iIdRechercher == iIdEnVerification)
                {
                    Debug.Log("iIdRechercher == iIdEnVerification");
                    int iTypeDuIdEnVerif = hMainCam.GetComponent<MainGame>().hTableJunction[j].GetComponent<Jonction>().iType;

                    if (iTypeDuIdEnVerif == 1)
                    {

                        Debug.Log("iTypeDuIdEnVerif == 1");
                        int iEtatDuIdEnVerif = hMainCam.GetComponent<MainGame>().hTableJunction[j].GetComponent<Jonction>().iEtat;

                        if (iEtatDuIdEnVerif >= 0)
                        {
                    ...
                    else
                    {
                        ...
                        funcDetectAcroche(hMainCam.GetComponent<MainGame>().hTableJunction[j]);
    ...
    void funcChangeFlechSens(int iSensPropagation, GameObject hLanceurFonction, int iEtatPropagation, int iActivation)
    {
            ...
            for (int i = 1; i < hMainCam.GetComponent<MainGame>().hTableJunction.Length; i++)
            {
                ...
                for (int j = 0; j < iLongeurTable; j++)
                {
                    ...
                    if (iIdVerif == iIdEnfantLanceurFonction && iTypeVerif != 1 && iTypeVerif != 2) // verife si l'ID de l'object verifier et le meme que l'id d'un des enfant du lancer de fonction // et que ce soit pas type architecture
                    {
                        ...
                        if (hMainCam.GetComponent<MainGame>().hTableJunction[i].GetComponent<Jonction>().bAttacher == true)
                        {
                        ...
                        if (hMainCam.GetComponent<MainGame>().hTableJunction[i].GetComponent<Jonction>().iIdEnfant.Length != 0) // si l'object modifier a des enfant on continue la propagation
                        {
                            ...
                            funcChangeFlechSens(iSensPropagation, hMainCam.GetComponent<MainGame>().hTableJunction[i], iEtatPropagation, iActivation);
    ...
    public void funcPropagationArchitecture(int iSensPropagation, GameObject hLanceurFonction, int iEtatPropagation, int iActivation)
    {
        ...
        if (iSensPropagation == 1 && iStackOverflod < 50)
        {
            ...
            if (iStackOverflod == 1 && gameObject.GetComponent<HingeJoint>() != null)
            {
                ...
                for (int j = 0; j < iLongeurTable; j++)
                {
                    ...
                    if (iIdVerif == iIdEnfantLanceurFonction && iTypeVerif != 1 && iTypeVerif != 2) // verife si l'ID de l'object verifier et le meme que l'id d'un des enfant du lancer de fonction // et que ce soit pas type architecture
                    {
                        ...
                        if (iEtatPropagation >= 0) // verifie si c'est une propagation de type positif ou négatif
                        {
                            ...
                            //-------------feedback join
                            if (iActivation >= 1 && hMainCam.GetComponent<MainGame>().hTableJunction[i].GetComponent<Jonction>().bAttacher == true)
                            {
    ...
    private void OnCollisionExit(Collision collision)
    {
        ...
        {
            ///////////////////////////////////////////////////////////////////////////////////
            if (Camera.main.GetComponent<MainGame>().bCri == false)
            {
