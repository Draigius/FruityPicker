...
public class MainGame : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        if (SceneManager.GetActiveScene().name == "Menu_Pancarte" || SceneManager.GetActiveScene().name == "Scene_Score")
        {
            AkSoundEngine.StopAll();
            GetComponent<AI_AudioMaster>().PlayEvent("Stop_All");
            GetComponent<AI_AudioMaster>().PlayEvent("onTheme");
        }
        else
        {
            AkSoundEngine.StopAll();
            GetComponent<AI_AudioMaster>().PlayEvent("Stop_All");
            GetComponent<AI_AudioMaster>().PlayEvent("onAmbiant");
        }
    }
    ...
    public void funcScore()
    {
        ...
        hLePanier.transform.position = new Vector3(0, -7.5f, 0);
    ...
    void Update()
    {
        if(bMenu == false)
        {
            ...
            if (fTimerGameDéconte <= 0)
            {

                funcLancementSceneFin();
    ...
    void funcUpdateUI()
    {
        ...
        textNgrappe.text = iGrappeRestant + "/" + iGrappeTotal;


        string tempo;

        if (Mathf.Floor(fTimerGameDéconte % 60)<10)
        {

            tempo = "0" + Mathf.Floor(fTimerGameDéconte % 60);
        }
        else
        {
            tempo = ""+Mathf.Floor(fTimerGameDéconte % 60);
        }

            textTimer.text = Mathf.Floor (fTimerGameDéconte/60) + ":" + tempo;
    ...
    void funcChangeGrape()
    {
        ...
        } else
        {
            ...
            hTableJunction[0].transform.position = new Vector3(0, 5.16f, 0);
